...
/-- Scheme morphism composition lifts through Option. -/
theorem ag_comp_option (f g : α → α) (v : Option α) :
    Option.map g (Option.map f v) = Option.map (g ∘ f) v := by
  cases v <;> simp

/-- Ring multiplication lifts through Option (for sheaf sections). -/
theorem ag_mul_comm [Mul α]
    (h : ∀ a b : α, a * b = b * a)
    (a b : Option α) : a * b = b * a := by
  cases a <;> cases b <;> simp [h]

/-- Ring associativity lifts through Option. -/
theorem ag_mul_assoc [Mul α]
    (h : ∀ a b c : α, a * b * c = a * (b * c))
    (a b c : Option α) : a * b * c = a * (b * c) := by
  cases a <;> cases b <;> cases c <;> simp [h]

/-- Distributivity lifts through Option. -/
theorem ag_distrib [Mul α] [Add α]
