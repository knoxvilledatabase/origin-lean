...
-- ============================================================================
-- Ring Laws: lifted through two cases
-- ============================================================================
...
theorem option_left_distrib [Mul α] [Add α]
    (h : ∀ a b c : α, a * (b + c) = a * b + a * c)
    (a b c : Option α) : a * (b + c) = a * b + a * c := by
  cases a <;> cases b <;> cases c <;> simp [h]

theorem option_right_distrib [Mul α] [Add α]
    (h : ∀ a b c : α, (a + b) * c = a * c + b * c)
    (a b c : Option α) : (a + b) * c = a * c + b * c := by
  cases a <;> cases b <;> cases c <;> simp [h]

theorem option_neg_mul [Mul α] [Neg α] (h : ∀ a b : α, -a * b = -(a * b))
    (a b : Option α) : -a * b = -(a * b) := by
  cases a <;> cases b <;> simp [h]

theorem option_mul_neg [Mul α] [Neg α] (h : ∀ a b : α, a * -b = -(a * b))
    (a b : Option α) : a * -b = -(a * b) := by
  cases a <;> cases b <;> simp [h]

theorem option_neg_neg [Neg α] (h : ∀ a : α, -(-a) = a)
    (a : Option α) : -(-a) = a := by
  cases a <;> simp [h]
